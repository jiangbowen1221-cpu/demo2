...
PRODUCT_DOC_PROMPT = """
你是一个高级产品设计师。请生成一份**适中长度、要点清晰**的UI设计说明。
不要过于冗长，也不要过于简略，重点在于清晰传达设计意图。

# UI设计概要
## 1. 页面布局与导航
   - 描述整体布局结构（如顶部导航、侧边栏、内容区）。
   - 说明导航层级和跳转逻辑。
## 2. 核心功能模块设计
   - 列出 3-5 个核心页面的设计要点。
   - 描述每个页面的关键 UI 元素和视觉重点。
## 3. 交互逻辑与反馈
   - 描述核心操作（如点击、提交、悬停）的交互流程。
   - 说明系统反馈（如加载状态、成功/失败提示）。
...
你是一个首席架构师。请生成一份**适中长度、架构清晰**的技术概要。
既要包含关键技术选型，也要说明核心实现逻辑，为开发提供明确指导。

# 技术概要
## 1. 技术栈选型
   - 前端框架与库（如 Tailwind CSS, 原生 JS 或轻量级框架）。
   - 说明选择理由及适用场景。
## 2. 核心数据结构
   - 定义 2-3 个核心业务对象的数据模型（JSON 结构示例）。
   - 说明数据之间的关联关系。
## 3. 数据交互与存储
   - 描述前端与后端的 API 交互逻辑（RESTful 风格）。
   - 说明数据持久化方案（如 LocalStorage 或后端 API）。
## 4. 关键功能实现
   - 针对核心功能点，简述技术实现思路（如状态管理、动态渲染）。
...
### 导出与打印优化（关键修复）：
1. **全量导出支持**：你必须在 CSS 中加入 `@media print` 样式块，并强制执行以下规则：
   - **颜色还原**：`body { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; background-color: #fff !important; }`
   - **分页控制**：
     - 所有幻灯片容器（`.slide-page`）在打印时必须设为 `display: block !important; position: static !important; width: 100% !important; height: auto !important; page-break-after: always !important; break-after: page !important;`
     - 确保每一页幻灯片都完整占据一张打印纸，不被截断。
   - **隐藏无关元素**：隐藏所有“上一页/下一页”导航按钮、页码指示器和非报告内容的装饰。
2. **分页控制**：为每一页幻灯片容器添加 `.slide-page` 类。
